...
add_action( 'wp_enqueue_scripts', function(){
    ...
    wp_enqueue_style( 'slick-theme-style', '//cdn.jsdelivr.net/npm/slick-carousel@1.8.1/slick/slick-theme.css', array('slick-style'));
    wp_enqueue_style( 'slider-style', THEME_URL.'/libs/mmm-slider/style.css', array('slick-style'), filemtime( dirname(__FILE__).'/style.css') );
...
@example：
    $id = "slideshow-1";
    $images = array(
        array( 'src' => THEME_URL.'/images/slide1.jpg', 'alt' => 'スライド１', 'link' => 'https://'  ),
...
function mmm_slider( $id, $images ){
    ob_start();
    ?>
    <ul id="<?php echo $id; ?>" class="mmm-slider">
        <?php
            $count = 1;
            foreach( $images as $image ){
                ?>
                   <li><?php if($image['link']){ ?><a href="<?php echo $image['link']; ?>"><?php } ?><img class="mmm-slider__img" src="<?php echo $image['src'] ? $image['src'] : ''; ?>" alt="<?php echo $image['alt'] ? $image['alt'] : '画像'.$count; ?>" /><?php if($image['link']){ ?></a><?php } ?></li>
                <?php
                $count ++;
            }
        ?>
